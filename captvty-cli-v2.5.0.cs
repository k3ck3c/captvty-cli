...
class CaptvtyCli
{
    ...
    static WorkerRun RunGetWorker(
        ...
        string title, string subtitle, string quality)
    {
        ProcessStartInfo psi = new ProcessStartInfo();
        psi.FileName = "mono";
        psi.Arguments =
        ...
        bool finished = p.WaitForExit(12 * 60 * 60 * 1000 + 60000);
    ...
    static int RunUpdate(string engine, string worker, string cachePath, string[] requested)
    {
        ...
        foreach (string channel in selected)
        {
            ...
            WorkerRun run = RunWorker(worker, "dump", engine, channel, "", 60000);
            if (run.TimedOut)
            {
    ...
    static int RunTargetedInfo(string channel, string query, string engine, string worker)
    {
        Console.Write("[1] " + channel + " ... ");
        Console.Out.Flush();
        WorkerRun run = RunWorker(worker, "info", engine, channel, query, 600000);
    ...
    static int RunSearchOrInfo(
        string mode, string query, string engine, string worker)
    {
        ...
        foreach (string channel in channels)
        {
            ...
            WorkerRun run =
                RunWorker(worker, mode, engine, channel, query, 60000);
    ...
    static int RunList(
        ...
        string engine, string worker)
    {
        WorkerRun run =
            RunWorker(worker, "list", engine, channel, query, 60000);
    ...
    static int Main(string[] args)
    {
        ...
        if (args.Length < 1)
        {
            Console.Error.WriteLine(
                "Usage:\n" +
                "  mono captvty-cli.exe update [--all | \"chaîne\" ...]\n" +
                "  mono captvty-cli.exe search [--live] \"texte\"\n" +
                "  mono captvty-cli.exe list   \"chaîne\" \"texte\"\n" +
                "  mono captvty-cli.exe info   \"texte\"\n" +
                "  mono captvty-cli.exe info   \"chaîne\" \"texte\"\n" +
        ...
        if (String.Equals(mode, "search", StringComparison.OrdinalIgnoreCase))
        {
            ...
            return RunSearchOrInfo("search", searchQuery, engine, worker);
        }
        ...
        if (String.Equals(mode, "update", StringComparison.OrdinalIgnoreCase))
        {
            ...
            return RunUpdate(engine, worker, cache, requested);
        }
        ...
        if (String.Equals(mode, "list", StringComparison.OrdinalIgnoreCase))
        {
            ...
            return RunList(args[1], String.Join(" ", args, 2, args.Length - 2), engine, worker);
        }
        ...
        if (String.Equals(mode, "info", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (args.Length >= 3)
                return RunTargetedInfo(args[1], String.Join(" ", args, 2, args.Length - 2), engine, worker);
            return RunSearchOrInfo("info", args[1], engine, worker);
        }
        ...
        if (String.Equals(mode, "get", StringComparison.OrdinalIgnoreCase))
        {
            ...
            WorkerRun run = RunGetWorker(worker, engine, args[1], args[2], args[3], args[4]);
